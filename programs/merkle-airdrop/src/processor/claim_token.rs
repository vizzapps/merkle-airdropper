...
pub struct Claim<'info> {
    ...
    /// CHECK: checked below
    #[account(mut)]
    pub recipient_mint_ata: AccountInfo<'info>,
    ...
    #[account(
        init,
        seeds = ["receipt".as_ref(), airdrop_state.key().as_ref(), recipient.key().as_ref(), &verification_data[0..8]],
        bump,
...
pub fn handle_claim(
    ...
    let recipient_mint_ata = &ctx.accounts.recipient_mint_ata;
    let airdrop_state = &ctx.accounts.airdrop_state;
    let vault = &ctx.accounts.vault;
    let payer = &ctx.accounts.payer;
    let token_program_id = &ctx.accounts.spl_token_program;
    ...
    if recipient_mint_ata.data_is_empty() {
        make_ata(
            recipient_mint_ata.to_account_info(),
